...
Here, we show the distribution of natural disasters grouped by subgroup and type which covered all the data since these two attributes do not have missing values.
```{r}
col_Subgroup <- categorization %>% group_by(Subgroup) %>% summarise(count=n()) %>% arrange(desc(count))
level_Subgroup <- col_Subgroup$Subgroup
cat_table <- categorization %>% 
  group_by(Subgroup,Type) %>% 
  summarise(count=n()) %>%
  arrange(desc(count), .by_group = TRUE) %>%
  arrange(factor(Subgroup, levels = level_Subgroup)) %>%
  mutate(percent = count/nrow(natural)*100) %>%
cat_table
```

```{r}
ggplot(cat_table,aes(x=fct_reorder(Type,-percent), y=percent)) +
  geom_bar(stat="identity") +
  theme(panel.grid.major.x = element_blank(),
    panel.grid.minor.x = element_blank()) +
  theme(axis.text.x = element_text(angle = 90,hjust = 1)) +
  labs(y="percentage",x="Type of disaster", title = "Ranking of Type of disaster")
